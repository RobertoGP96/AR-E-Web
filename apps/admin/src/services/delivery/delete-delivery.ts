...
export const deleteDelivery = async (id: number): Promise<void> => {
  return await apiClient.delete<void>(`/api_data/delivery_receips/${id}/`);
};
...
export const deleteMultipleDeliveries = async (ids: number[]): Promise<void> => {
  return await apiClient.post<void>('/api_data/delivery_receips/bulk-delete/', { delivery_ids: ids });
};