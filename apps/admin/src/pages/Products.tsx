...
export default function Products() {
  ...
  const apiFilters = useMemo<ApiProductFilters>(() => {
    ...
    if (productFilters.category && productFilters.category !== "all")
      f.category = productFilters.category;
    if (productFilters.status && productFilters.status !== "all")
      f.status = productFilters.status;
    if (productFilters.shop && productFilters.shop !== "all")
      f.shop = Number(productFilters.shop);
    if (
      productFilters.price_min !== "" &&
      productFilters.price_min !== undefined
    )
      f.price_min = Number(productFilters.price_min);
    if (
      productFilters.price_max !== "" &&
      productFilters.price_max !== undefined
    )
      f.price_max = Number(productFilters.price_max);
    return f;
  }, [productFilters]);

  // Obtener productos desde la API con filtros
  const { products, isLoading, error, isFetching } = useProducts(apiFilters);
