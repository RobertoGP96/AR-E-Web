...
import { CreditCard, Loader2, Package, Trash2, ShoppingCart, DollarSign } from 'lucide-react';
import { shoppingReceipService } from '@/services/api';
import type { CreateProductBuyedData, ProductBuyed, ShoppingReceip } from '@/types/models';
...
export function PurchaseForm({ onSuccess, onCancel }: PurchaseFormProps) {
  ...
  const [selectedProductsDetails, setSelectedProductsDetails] = useState<ProductBuyed[]>([]);
  ...
  const handleCartChange = (apiData: CreateProductBuyedData[], fullDetails?: ProductBuyed[]) => {
  ...
  const removeProductFromCart = (productId: string) => {
    const newDetails = selectedProductsDetails.filter(item => item.product_id !== productId);
    setSelectedProductsDetails(newDetails);

    const newApiData = selectedProducts.filter(item => item.original_product !== productId);
    setSelectedProducts(newApiData);
  };

  // Actualizar cantidad de un producto
  const updateProductQuantity = (productId: string, delta: number) => {
    const newDetails = selectedProductsDetails.map(item => {
  ...
  // Calcular resumen
  const cartSummary = {
    totalItems: selectedProductsDetails.reduce((sum, item) => sum + item.amount_buyed, 0),
            ...
            render={({ field }) => (
              <FormItem>
                ...
                >
                  ...
                  <SelectContent>
                    {shops.map((shop) => (
                      <SelectItem key={shop.id} value={shop.id.toString()} className='capitalize'>
                        {shop.name}
                      </SelectItem>
                    ))}
                  </SelectContent>
        ...
        <div className='w-full mt-4 space-y-3'>
          <div className='w-full flex flex-row items-center justify-between'>
            <div className="flex items-center gap-2">
              <h2 className="text-base font-semibold">Productos de la Compra</h2>
              {selectedProductsDetails.length > 0 && (
                <Badge variant="default">
                  {selectedProductsDetails.length} {selectedProductsDetails.length === 1 ? 'producto' : 'productos'}
                </Badge>
              )}
            </div>
            ...
            <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
              <DialogTrigger asChild>
                <Button
                  ...
                  className={!selectedShopId ? 'pointer-events-none opacity-50' : ''}
                >
              ...
              <DialogContent className="min-w-[70%] max-h-[90vh]">
                <DialogHeader>
                  <DialogTitle>Seleccionar Productos para la Compra</DialogTitle>
                </DialogHeader>
                <ProductSelector
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 text-xs text-muted-foreground flex-wrap">
                          <span className="font-mono">{item.original_product_details.sku}</span>
                      ...
                      <div className="flex items-center gap-3 shrink-0">
                        ...
                        <div className="flex items-center gap-1 border rounded-md p-1">
                          <Button
                            ...
                            size="icon"
                            className="h-6 w-6"
                            onClick={() => updateProductQuantity(item.product_id as string, -1)}
                          >
                            -
                          </Button>
                        ...
                        <Button
                          ...
                          onClick={() => removeProductFromCart(item.product_id as string)}
                        >
