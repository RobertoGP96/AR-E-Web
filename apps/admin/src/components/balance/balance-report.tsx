        ...
        <Card className="border-l-4 border-l-green-500 hover:shadow-lg transition-shadow">
          ...
          <CardContent className="pt-4">
            {deliveryAnalysisError ? (
              ...
              <div className="space-y-6">
                <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
                  <MetricCard label="Entregas - Peso" value={(deliveryAnalysis?.count || 0)+`-  ${((deliveryAnalysis?.total_weight ?? 0) || 0).toLocaleString()} lb`} />

                  <MetricCard label="Ingresos" value={formatUSD(deliveryAnalysis?.total_delivery_revenue || 0)} />
                  <MetricCard label="Gastos" value={formatUSD((deliveryAnalysis?.total_delivery_expenses || 0))} />
                  <MetricCard label="Agentes" value={formatUSD((deliveryAnalysis?.total_manager_profit || 0))} />
                        ...
                        <TableBody>
                          {Object.entries(deliveryAnalysis.deliveries_by_category).map(([category, values]) => (
                            <TableRow key={category} className="hover:bg-muted/50 transition-colors">
                              ...
                              <TableCell className="text-right text-blue-600 font-semibold">{formatUSD(values.total_delivery_revenue || 0)}</TableCell>
                              <TableCell className="text-right text-red-600 font-semibold">{formatUSD(values.total_delivery_expenses || 0)}</TableCell>
                              <TableCell className="text-right text-red-600 font-semibold">{formatUSD(values.total_manager_profit || 0)}</TableCell>
                              <TableCell className="text-right text-green-600 font-semibold">{formatUSD(values.total_system_profit || 0)}</TableCell>
                            </TableRow>
                          ))}
                        </TableBody>
                      </Table>
            ...
            {invoicesRangeData ? (
              <div className="space-y-4">
                <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
                  ...
                  <MetricCard label="Costo Pesajes" value={formatUSD(invoicesRangeData.total_tag_subtotal)} />
            ...
            {expenseError ? (
              ...
              <div className="space-y-6">
                <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                  <MetricCard label="Total" value={expensesAnalysis.count || 0} />
                ...
                <div className="border-t pt-4">
                  ...
                  <div className="space-y-0.5">
                    {Object.entries(expensesAnalysis.expenses_by_category || {})
                      ...
                      .map(([category, value]) => (
                        <div key={category} className="flex items-center justify-between p-0.5 rounded-md hover:bg-muted/50 transition-colors">
...
function ExecutiveSummary({
  ...
  const summary = React.useMemo(() => {
    ...
    const operationalExpenses = purchasesAnalysis?.total_operational_expenses || 0;
    ...
    const orderProfit = (ordersAnalysis?.total_revenue || 0) - (ordersAnalysis?.total_cost || 0);
    const deliveryProfit = (deliveryAnalysis?.total_system_profit || 0);
    const purchaseProfit = (purchasesAnalysis?.total_purchase_amount || 0) - (purchasesAnalysis?.total_real_cost_paid || 0);
    ...
    <>
      ...
      <CardContent className="space-y-6">
        ...
        <div className="border-b pb-6">
          ...
          <div className="mt-4 p-3 rounded-lg border border-gray-200">
            <div className="flex justify-between items-center">
              <span className="font-semibold text-gray-900">Ingreso Total:</span>
              <span className="text-xl font-bold text-gray-700">{formatUSD(summary.totalIncome)}</span>
            </div>
          </div>
        </div>
        ...
        <div className="border-b pb-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            <MetricCard label="Compras" value={formatUSD(summary.purchaseCosts)} />
            <MetricCard label="Facturas" value={formatUSD(summary.invoiceCosts)} />
            <MetricCard label="Gastos" value={formatUSD(summary.totalExpenses)} />
          </div>
          <div className="mt-4 p-3 rounded-lg  border border-red-200">
            <div className="flex justify-between items-center">
              <span className="font-semibold text-red-900">Pasivo Total:</span>
              <span className="text-xl font-bold ">{formatUSD(summary.totalCosts)}</span>
            </div>
          </div>
        </div>
        ...
        <div className="border-b pb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div className={cn(
              ...
              <div className="text-sm font-medium text-muted-foreground mb-2">Ganancia Órdenes</div>
              <div className={cn(
                'text-2xl font-bold',
                summary.orderProfit >= 0 ? 'text-blue-600' : 'text-orange-600'
              )}>
                {formatUSD(summary.orderProfit)}
              </div>
              <div className="text-xs text-muted-foreground mt-1">
                {((summary.orderProfit / summary.totalOrderRevenue) * 100).toFixed(1)}% margen
              </div>
            </div>
            ...
            <div className={cn(
              ...
              <div className="text-sm font-medium text-muted-foreground mb-2">Ganancia Entregas</div>
              <div className={cn(
                'text-2xl font-bold',
                summary.deliveryProfit >= 0 ? 'text-purple-600' : 'text-orange-600'
              )}>
                {formatUSD(summary.deliveryProfit)}
              </div>
              <div className="text-xs text-muted-foreground mt-1">
                {((summary.deliveryProfit / summary.totalDeliveryRevenue) * 100).toFixed(1)}% margen
              </div>
            </div>
            ...
            <div className={cn(
              ...
              <div className="text-sm font-medium text-muted-foreground mb-2">Ganancia Compras</div>
              <div className={cn(
                'text-2xl font-bold',
                summary.purchaseProfit >= 0 ? 'text-amber-600' : 'text-orange-600'
              )}>
                {formatUSD(summary.purchaseProfit)}
              </div>
              <div className="text-xs text-muted-foreground mt-1">
                {((summary.purchaseProfit / (purchasesAnalysis?.total_purchase_amount || 1)) * 100).toFixed(1)}% margen
              </div>
            </div>
            ...
            <div className={cn(
              ...
              <div className="text-xs text-muted-foreground mt-1">
                {((summary.netProfit / (ordersAnalysis?.total_revenue || 1)) * 100).toFixed(2)}% margen
              </div>
            </div>
          </div>
        </div>
