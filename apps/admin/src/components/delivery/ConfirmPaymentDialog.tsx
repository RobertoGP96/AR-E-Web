...
export function ConfirmPaymentDialog({
  ...
  const costoEntrega = delivery
    ? Math.max(0, delivery.weight_cost - delivery.payment_amount - (delivery.balance_applied || 0))
    : 0;
  const nombreCliente = delivery?.client
    ? `${delivery.client.name} ${delivery.client.last_name || ""}`.trim()
    : "Cliente";

  const monto = parseFloat(montoPagado) || 0;
  const saldoAplicado = usarSaldo ? Math.min(saldoCliente, costoEntrega) : 0;
  const totalCubierto = monto + saldoAplicado;
  const diferencia = totalCubierto - costoEntrega;
  const pendiente = Math.max(0, costoEntrega - totalCubierto);

  // Si el cliente paga de más, el excedente se suma al saldo restante
  const excedente = diferencia > 0 ? diferencia : 0;
  const saldoDeshabilitado = saldoCliente <= 0 || monto >= costoEntrega;
  ...
  const estadoCalculado = () => {
    ...
    if (totalCubierto >= costoEntrega) return "pagado";
    return "parcial";
  };
  ...
  const porcentaje =
    costoEntrega > 0 ? Math.min(100, (totalCubierto / costoEntrega) * 100) : 0;
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      await onConfirm(delivery.id!, monto, paymentDate, payStatus, usarSaldo ? saldoAplicado : undefined);
    } catch {
  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="max-w-md p-0 overflow-hidden border-none shadow-2xl bg-transparent sm:rounded-[2rem]">
        <DialogTitle className="sr-only">Panel de Pago de Entrega</DialogTitle>
        <div className="w-full bg-white sm:rounded-[2rem] overflow-hidden shadow-xl shadow-orange-100 flex flex-col max-h-[90vh]">
          ...
          <div className="overflow-y-auto px-6 py-5 space-y-5 custom-scrollbar">
            ...
            <div className="rounded-xl text-white overflow-hidden shadow-lg mt-2 relative border border-orange-200">
              ...
              {totalCubierto >= costoEntrega && costoEntrega > 0 && (
