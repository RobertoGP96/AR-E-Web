...
import { useAddProductsToPackage } from '@/hooks/package/useAddProductsToPackage';
import type { Product } from '@/types/models';
import type { ProductFilters } from '@/types/api';
import { Badge } from '@/components/ui/badge';
import { toast } from 'sonner';

interface SelectedProductsForPackageProps {
  packageId: number;
  onProductsAdded?: () => void;
}

const SelectedProductsForPackage: React.FC<SelectedProductsForPackageProps> = ({
  packageId,
  onProductsAdded,
}) => {
  ...
  const { mutateAsync: addProducts, isPending: isAdding } = useAddProductsToPackage();
  ...
  const handleAddProducts = async () => {
    ...
    try {
      const productsData = Array.from(selectedProducts.values()).map(({ product, amount }) => ({
        original_product_id: product.id,
        amount_received: amount,
        observation: '',
      }));

      await addProducts({ packageId, products: productsData });
      toast.success(`${selectedProducts.size} producto(s) agregado(s) al paquete`);
      setSelectedProducts(new Map());
      setOpen(false);
      onProductsAdded?.();
    } catch (error) {
      console.error('Error adding products to package:', error);
      toast.error('Error al agregar productos al paquete');
    }
  };
...
export default SelectedProductsForPackage;
