...
export function BuildDeliveriesStep({
  groups,
  categoryOptions,
  canWrite,
  onGoToPackages,
}: BuildDeliveriesStepProps) {
  ...
  function renderProduct(p: PrepareProduct) {
    const qty = qtyOf(p);
    const picked = qty > 0;
    const unpurchased = p.requested - p.purchased;
    return (
      <li key={p.id}>
        <div
      ...
      <div className="surface-card animate-in fade-in duration-300 flex flex-col items-center gap-3 p-12 text-center">
        ...
        <div>
          ...
          <p className="mx-auto mt-1 max-w-md text-sm text-muted">
            Cuando marques llegadas al revisar los paquetes, los productos
          ...
          {!selectedGroup ? (
            <div className="surface-card flex flex-col items-center justify-center gap-2 p-12 text-center">
              ...
              <p className="max-w-sm text-sm text-muted">
                ...
                recibidos y armar su entrega. Puedes crear varias entregas
                según vaya llegando la mercancía.
              </p>
            </div>
          ) : (
            <div
                ...
                {selectedGroup.totalIncoming > 0 ? (
                  <div className="mt-3 flex flex-wrap items-center gap-2 rounded-lg border border-warning-soft-foreground/25 bg-warning-soft px-3 py-2 text-xs text-warning-soft-foreground">
                    ...
                    <span>
                      ...
                      camino. Puedes crear ahora una entrega parcial con lo
                      disponible y otra cuando llegue el resto.
                    </span>
                  </div>
                ) : null}
              </header>
