...
| `Package` (`api_package`) | `agencyName`, `numberOfTracking` único, `statusOfProcessing` (default `Enviado`), `arrivalDate`, `packagePicture?`, `packagePicture2?` (2 fotos, sept 2026) | `packageProducts` (ProductReceived) | estado: `Enviado`, `Recibido`, `Procesado` |
...
| `delivery` | admin, agent, logistical (agente solo lectura y **solo sus clientes asignados**: la lista, el detalle y `/delivery/prepare` filtran por `assignedAgentId`; las actions de escritura exigen admin/logistical) |
...
Grupos: `(auth)` para login (sin shell), `(admin)` para el panel (con sidebar/header), `(print)` para documentos imprimibles (sin shell; `layout.tsx` re-verifica sesión y `isStaff`), más `/unauthorized` y `/api/auth`.
...
| `/dashboard` | `src/app/(admin)/dashboard/page.tsx` + `dashboard-greeting.tsx` (+ `components.tsx` y `admin/agent/accountant/logistical-dashboard.tsx`) | Dashboard adaptado al rol: atajos, métricas y listas operativas por rol | staff |
...
| `/orders` | `src/app/(admin)/orders/(list)/page.tsx` + `orders-client.tsx` | Listado/CRUD de órdenes + pago | admin, agent |
...
| `/purchases` | `src/app/(admin)/purchases/(list)/page.tsx` + `purchases-client.tsx` | CRUD de compras (ShoppingReceip) | admin |
| `/purchases/[id]` | `src/app/(admin)/purchases/[id]/page.tsx` + `purchase-detail-client.tsx` | Productos comprados + reembolsos | admin |
| `/packages` | `src/app/(admin)/packages/(list)/page.tsx` + `packages-client.tsx` | CRUD paquetes + cambio rápido de estado | admin, logistical |
| `/packages/[id]` | `src/app/(admin)/packages/[id]/page.tsx` + `package-detail-client.tsx` | Recepción de productos | admin, logistical |
| `/delivery` | `src/app/(admin)/delivery/(list)/page.tsx` + `delivery-client.tsx` | CRUD entregas + pago | admin, agent (ver), logistical |
...
| `/delivery/prepare` | `src/app/(admin)/delivery/prepare/page.tsx` + `prepare-client.tsx` (+ `review-packages-step.tsx`, `bags-step.tsx`) | Mesa de preparación en 2 fases (tabs con estado conservado): **(1) Revisar paquetes** — paquetes pendientes uno a uno con checklist de llegadas por producto y cantidad (lote `registerArrivalsAction`; cada unidad marcada **cae sola en la bolsa abierta** de su cliente+categoría; candidatos sin categoría bloqueados con aviso; toast con las bolsas destino; Enviado→Recibido automático; «Terminar revisión» → Procesado); **(2) Bolsas** — bolsas abiertas por cliente (una por categoría) con su contenido: sacar productos (`adjustBagItemAction`), embolsar recibidos sueltos (`addLooseToBagAction`, individual o todo), y **pesar y cerrar** cada bolsa (`registerBagWeightAction`, con costo/ganancia en preview y confirmación); contexto de entregas ya pesadas sin entregar y unidades en camino. Agente: solo lectura de sus clientes asignados | admin, agent (ver), logistical |
...
| `/client-balances` | `src/app/(admin)/client-balances/page.tsx` | Redirige a `/users?tab=balances` (`users/balances-client.tsx`): balance por cliente agregado en vivo + botón «Generar factura» (`users/client-invoice-dialog.tsx`) | admin, accountant |
| `/users/[id]/statement` | `src/app/(print)/users/[id]/statement/page.tsx` + `statement-document.tsx` (+ `(print)/print-toolbar.tsx`) | Documento imprimible al cliente (A4, `window.print()` → PDF del navegador): `?mode=pending&items=o12,d7` factura de pendientes; `?mode=history&from&to` estado de cuenta tipo extracto; `?mode=orders&orders=12,15` factura por pedidos con productos. Lógica pura en `src/lib/client-statement.ts`, carga en `src/lib/client-statement-data.ts`. Nada se persiste | admin, accountant |
...
| `/settings/import` | `src/app/(admin)/settings/import/page.tsx` + `import-client.tsx` (antes `/import`; esa ruta ahora redirige aquí) | Importación de embarques desde Excel "AR&E Shipps #NNN" (subir → previsualizar/omitir → importar). Parser/mapeador en `src/lib/excel-import/` (exceljs); actions `analyzeExcelAction`/`runImportAction` (transacción única: shops, cuentas, agentes, clientes con teléfono placeholder `imp-…`, órdenes por cliente, productos con descripción/cantidad/precio, ShoppingReceip por fila "Factura" con coste real **más agrupación automática tienda+cuenta+fecha** para filas sin pedido — ningún producto queda sin compra; si el grupo no trae cuenta se usa una cuenta "Sin cuenta" por tienda —, paquetes identificados por **ID de paquete (columna) + nro. de rastreo** (solo etiqueta → se antepone el tag del embarque), recepciones si la fila tiene paquete o F. llegada, gastos de la hoja General). Tarifas de tienda replicadas del Excel: Shein 7 % IVA, Amazon +1 %, Temu +3 %, Otras +5 %. Scripts de diagnóstico: `scripts/inspect-import.ts` y `scripts/dry-run-import.ts` (npx tsx). | admin |
...
**Dashboard** — `force-dynamic`. Adaptado al rol (agosto 2026): `page.tsx` lee la sesión y enruta a un server component por rol (`admin-dashboard.tsx`, `agent-dashboard.tsx`, `accountant-dashboard.tsx`, `logistical-dashboard.tsx`), cada uno con sus propias agregaciones Prisma en paralelo. Piezas compartidas en `dashboard/components.tsx`: `QuickActions` (tarjetas-link de atajos filtradas con `canAccessPath`), `StatLink` (envuelve `StatCard` en un link a la vista filtrada si el rol puede abrirla), `ListCard`/`RowLink`/`Row`/`ListEmpty` (listas operativas), `SectionTitle`, `DashboardHeading`, `fmtMoney`. `DashboardGreeting` (client: saludo según hora, tarjeta "Tasa Cambio", reloj en vivo) con subtítulo por rol. Contenido por rol:
- **admin**: atajos a 8 áreas + "Pulso Operativo" (entregas pendientes/en tránsito, paquetes por procesar, mercancía lista, órdenes activas) + los 3 grupos históricos ("Resumen General", "Finanzas", "Clientes & Operaciones"), ahora con las cards linkadas a sus vistas filtradas (`/delivery?status=…`, `/users?tab=balances`, etc.).
- **agent**: cartera (clientes asignados, clientes con deuda, órdenes activas y por cobrar de `salesManagerId`), comisiones (suma de `managerProfit` de entregas de sus clientes asignados, total y mes), mercancía lista de sus clientes; listas "Entregas por recoger de mis clientes" (Pendiente/En transito, con comisión por entrega) y "Mis órdenes recientes"; atajos con `/orders?manager=<id>`.
- **accountant**: resultados (ingresos, ganancia, margen, reembolsos), cuentas por cobrar (órdenes sin cobrar, entregas sin pagar, deudas/saldos de clientes), mes en curso (ingresos, compras, gastos, facturas de envío); listas "Últimos gastos" y "Gastos del mes por categoría" (groupBy con barras).
- **logistical**: entregas (hoy/pendientes/en tránsito/fallidas), cobros y carga (por cobrar, cobrado del mes, entregadas del mes con peso, comisiones generadas), paquetes y mercancía (en camino, por revisar, mercancía lista); listas "Clientes con entregas por recoger" (recibido sin entregar agrupado por cliente, misma base que `/delivery/prepare`) y "Últimas entregas" (cobro + comisión por entrega).
Sin gráficas (las gráficas viven en Analytics).
...
**Órdenes (lista)** — Búsqueda por cliente/teléfono; `FilterPopover` con estado y tipo de pago; tabla desktop (`lg:`) con columnas Cliente (link al detalle), Gestor, Estado (`OrderStatusBadge`), Pay (`PayStatusBadge`), Productos, Total, Creado, Acciones (botón `$` confirmar pago — deshabilitado visualmente si ya está Pagado, con toast informativo —, Open, editar, eliminar); lista de cards en móvil. Diálogos: `order-dialog.tsx` (cliente, gestor opcional, estado, recibido, saldo aplicado, observaciones; el total es derivado de los productos), `delete-dialog.tsx`, `confirm-payment-dialog.tsx` (envuelve `PaymentPanel`). **Gestor de la orden (ADR-0007, sept 2026):** cualquier miembro del personal (`SALES_MANAGER_ROLES` en `lib/order-manager.ts`, etiqueta con rol salvo agentes); en `/orders/new` y en `order-dialog.tsx` viene preseleccionado el admin general (`lib/general-admin.ts`: admin activo superusuario o el más antiguo) y **el gestor ya no filtra los clientes** (el agente asignado aparece como descripción del cliente). `orders/actions.ts` `resolveManager` valida que sea personal activo.
...
**Balance de Clientes (`/users?tab=balances`, antes `/client-balances`)** — Sin mutaciones. Recalcula el balance **en vivo** por agregación (misma fórmula que `recalculate_balance`) y lo contrasta con la columna cacheada; tarjetas resumen (deuda total, saldo a favor total, clientes), filtro deuda/favor/al día, orden por mayor deuda primero, `BalanceBadge` (DEUDA roja / SALDO A FAVOR esmeralda / AL DÍA azul). **Generar factura (sept 2026):** botón por fila que abre `client-invoice-dialog.tsx`; carga con `loadClientInvoiceOptionsAction` las partidas pendientes, los pedidos y el rango con movimientos, y ofrece tres tipos en tabs: **Pendientes** (checkboxes sobre órdenes/entregas con `pendiente = costo − efectivo − saldo aplicado`, todas marcadas por defecto), **Estado de cuenta** (extracto tipo cuenta bancaria con débitos, créditos y saldo corriente = RN-021; el saldo aplicado se lista como informativo sin mover el saldo; rango de fechas opcional que resume lo anterior como saldo inicial) y **Por pedidos** (pedidos elegidos con sus productos: cantidad, precio unitario, total RN-001, pagado y pendiente por pedido). «Generar documento» abre `/users/[id]/statement` en pestaña nueva; desde su barra se imprime o se guarda como PDF (`@media print` en `globals.css`). Referencia determinista `FP|EC|FO-<cliente>-<fecha>`; no es el `Invoice` del transportista ni se guarda en BD. Tests: `src/lib/client-statement.test.ts`.
...
**Shell del admin** (`(admin)/layout.tsx`): sidebar + header + `<main>`; navegación en `admin-nav.tsx` (grupos: Dashboard / Gestión / Órdenes y Productos / Logística / Finanzas + inferior Configuración, filtrada por rol), `mobile-nav.tsx` (drawer, con blur del fondo), `bottom-nav.tsx` (barra inferior flotante móvil, glass), `breadcrumbs.tsx` (mapa ruta→etiqueta/icono, detecta IDs numéricos/UUID para "Detalles de X"), `notifications-bell.tsx` (campana con polling de 60 s vía server action, badge animado, dropdown con dot de prioridad, marcar una/todas leídas), `sign-out-button.tsx`.

**Catálogo de navegación** (`src/lib/navigation.ts`, con tests): secciones (href, etiqueta, etiqueta corta, icono) compartidas por sidebar, drawer y barra inferior. `matchSection(pathname)` resuelve la sección activa por el href más específico (`/delivery/prepare` ≠ `/delivery`; `/orders/123` cuenta como Órdenes; `/settings/*` como Configuración). La **barra inferior** tiene 4 accesos fijos por rol (`BOTTOM_NAV_BY_ROLE`, siempre filtrados por `canAccessPath`) + un quinto hueco que abre el drawer (estado compartido en `mobile-nav-context.tsx`). Ese hueco es **contextual**: si la ruta actual no es un acceso fijo (Ajustes, Perfil, Usuarios para un contable, Preparar para un admin…) muestra esa sección marcada activa, así la barra nunca queda sin selección.

**Feedback de navegación — regla**: nada de spinners en los menús. La sección pulsada se marca activa al instante (`use-optimistic-pathname.ts`, mismo patrón de estado optimista que SettingsNav/UsersTabs; ignora clics con modificador) y el contenido lo cubre el esqueleto `loading.tsx` de la ruta destino (`ListPageSkeleton` para listas: cabecera + toolbar + tabla/tarjetas + paginación; `columns` aproxima la tabla real). En Next 16 el límite de `loading.tsx` no se repite al cambiar solo search params (filtros/paginación), así que todas las listas tienen esqueleto sin parpadear al filtrar. Las secciones con rutas hijas que ya tienen su propio esqueleto (`orders`, `packages`, `purchases`, `delivery`) llevan la página de lista dentro de un route group `(list)/` para que su `loading.tsx` no envuelva también `[id]`/`prepare` (evita el doble esqueleto). `settings/loading.tsx` pinta solo el contenido: la cabecera y las pestañas las pone el layout y se mantienen fijas.
...
| `registerBagWeightAction` | ídem | Pesar una bolsa de `/delivery/prepare`: fija `weight` (> 0, lo que **cierra** la bolsa), deriva `weightCost`/`managerProfit`, recalcula payStatus contra pagos ya registrados y balance del cliente | DeliverReceip, Category, CustomUser | peso > 0; solo entregas «Pendiente» |
| `adjustBagItemAction` | ídem | Fijar/quitar (amount 0) una fila de una **bolsa abierta** (Pendiente + peso 0); recompute del producto; si la bolsa queda vacía se borra | ProductDelivery, DeliverReceip, Product | entero ≥ 0; solo bolsas abiertas; incremento ≤ recibido − entregado |
| `addLooseToBagAction` | ídem | Echar recibidos sueltos a la bolsa abierta de su cliente+categoría (la crea si no existe); transaccional, devuelve resumen de bolsas destino | DeliverReceip, ProductDelivery, Product | sin duplicados; `amount ≤ recibido − entregado`; producto con categoría |
...
| `addReceivedProductAction` / `removeReceivedProductAction` | ídem | Recepción/eliminación de `ProductReceived` + recompute, transaccional. La recepción **auto-embolsa** (misma bolsa abierta que `registerArrivalsAction`); la eliminación retira las unidades de las bolsas abiertas del cliente+categoría (`pullUnitsFromOpenBags`) y se **bloquea** si ya están en una entrega pesada/despachada (dejaría entregado > recibido) | ProductReceived, Product, Package, DeliverReceip, ProductDelivery | `amount ≤ comprado − recibido`; observación ≤ 200; producto con categoría para recibir |
| `registerArrivalsAction` | ídem | Lote de la fase «Revisar paquetes» de `/delivery/prepare`: N recepciones en un paquete en una sola transacción, cada unidad **cae en la bolsa abierta** de su cliente+categoría (`addUnitsToOpenBag`, se crea si falta), recompute por producto, cambio opcional de estado del paquete (Enviado → Recibido); devuelve `bags` (resumen de bolsas destino para el toast) | ProductReceived, Product, Package, DeliverReceip, ProductDelivery | `arrivalBatchSchema`; sin productos duplicados; `amount ≤ comprado − recibido` por ítem; observación ≤ 200; todos los productos con categoría |
...
| `loadClientInvoiceOptionsAction` | `(admin)/users/invoice-actions.ts` | Solo lectura: partidas pendientes, pedidos con totales, balance en vivo y rango con movimientos de un cliente para el diálogo «Generar factura» | — | `ROLES.finance`; `parseId` |
...
**`src/lib/open-bags.ts`** — «Bolsas abiertas» del flujo de preparación: una **DeliverReceip en `Pendiente` con `weight = 0`** hace de bolsa física del cliente para una categoría (no hay tabla propia: la BD la posee Django y no se migra; el estado se deriva por completo de status + weight). `addUnitsToOpenBag` busca/crea la bolsa del cliente+categoría y acumula la fila `ProductDelivery` (merge por producto); `countUnitsInOpenBags` / `pullUnitsFromOpenBags` sirven al deshacer recepciones (retiran de las bolsas más nuevas primero); `deleteBagIfEmpty` borra bolsas Pendiente/peso 0 sin filas ni pagos; `describeBags` formatea el resumen para toasts. **Pesar la bolsa (weight > 0) la cierra**: deja de recibir auto-llenado y las llegadas posteriores abren otra. Consecuencia asumida: un producto embolsado cuenta como `amount_delivered` → su estado pasa a `Entregado` aunque la entrega siga `Pendiente`; el estado real de cara al cliente es el de la entrega.
...
2. En `/orders/[id]` añade productos; el formulario muestra la cascada de costos en vivo y el servidor la recalcula (fuente de verdad). La **categoría es obligatoria** (`productFormSchema`): decide en qué bolsa (entrega) cae el producto al procesarse.
...
3. En `/packages/[id]` registra recepciones (tope: comprado − recibido; el producto debe tener categoría) → cada unidad recibida **cae sola en la bolsa abierta** de su cliente+categoría. Flujo recomendado: la fase «Revisar paquetes» de `/delivery/prepare` hace lo mismo en lote — checklist multi-producto sobre el paquete abierto (`registerArrivalsAction`), pensada para paquetes que llegan incompletos: lo no marcado sigue pendiente y se recibe cuando llegue en otro paquete/división. Deshacer una recepción retira las unidades de la bolsa; si ya están en una entrega pesada, se bloquea.
...
4. Flujo recomendado: `/delivery/prepare` en 2 fases. Fase 1 «Revisar paquetes»: se abre cada paquete pendiente y se marca qué productos llegaron y cuántas unidades (`registerArrivalsAction` en lote; un producto puede llegar repartido en varios paquetes — p. ej. 150 compradas que llegan 95 + 25 + 20); **cada unidad marcada cae automáticamente en la bolsa abierta de su cliente+categoría** (entrega `Pendiente` con peso 0, una por cliente y categoría, creada al vuelo) — espejo digital de ir echando la mercancía en la bolsa física de cada cliente. Fase 2 «Bolsas»: por cliente se ven las bolsas abiertas con su contenido (sacar un producto que cayó mal con `adjustBagItemAction`, embolsar recibidos sueltos con `addLooseToBagAction`) y se **pesa cada bolsa** cuando está completa (`registerBagWeightAction`): el peso fija `weightCost`/`managerProfit` y **cierra la bolsa**, así que la mercancía que llegue después de esa categoría abre otra bolsa (entregas parciales incrementales). Un cliente puede tener varias entregas pendientes a la vez, una por categoría.
